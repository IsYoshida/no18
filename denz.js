...
memberLimit = 2
ban = []
premium = ["557491143362@s.whatsapp.net","557491956806@s.whatsapp.net"]
...
async function starts() {
	...
	denz.on('connecting', () => {
		start('2', 'conectando a Android Número 18...')
	})
	denz.on('open', () => {
		success('2', 'ja esta conectado!')
	})
		...
		if(antifake.includes(anu.jid)) {
			if (anu.action == 'add'){
				...
				if(!num.split('@')[0].startsWith(55)) {
					denz.sendMessage(mdata.id, 'Esse número aqui não! 😡', MessageType.text)
		...
		try {
			...
			if (anu.action == 'add') {
				...
				try {
					...
					ppimg = 'http://imgur.com/a/rmHlD87'
				}
				teks = `Oʟᴀ́ @${num.split('@')[0]}!\nSᴇᴊᴀ ʙᴇᴍ-ᴠɪɴᴅᴏ(ᴀ).*${mdata.subject}*\n\nʟᴇɪᴀ ᴀs ʀᴇɢʀᴀs  ɴᴀ ᴅᴇsᴄʀɪᴄ̧ᴀ̃ᴏ ᴅᴏ ɢʀᴜᴘᴏ ᴇ ᴅɪᴠɪʀᴛᴀ-sᴇ*

Dɪɢɪᴛᴇ ${prefix}daftar ɴᴏ ᴄʜᴀᴛ ᴘʀɪᴠᴀᴅᴏ ᴘᴀʀᴀ ʀᴇɢɪsᴛʀᴀʀ-sᴇ ᴇ ᴄᴏᴍᴇᴄᴀʀ ᴀ ᴜsᴀʀ  ʙᴏᴛ!`
				...
				try {
					...
					ppimg = 'http://imgur.com/a/FcWoiqb'
				}
				teks = `*Parabéns!*\n@${num.split('@')[0]}.Agora você faz parte da lista de admins do grupo!`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
				...
				try {
					...
					ppimg = 'http://imgur.com/a/kOzirAS'
				}
				teks = `*Você não é mais admin*\n@${num.split('@')[0]}.`
				let buff = await getBuffer(ppimg)
				denz.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
	...
	denz.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '*Desculpe, este comando só pode der usado em grupos!*',
					benned: '',
					ownerG: '*Este comando só pode ser usado pelos administradores do grupo!*',
					ownerB: '*Este comando só pode ser usado por meu proprierário!* ',
					premium: '*Este recurso só está disponível para usuários premium!*',
					userB: `Olá *${pushname2}*, Você não está registrado na minha lista, digite \n*${prefix}daftar* _no chat privado para registrar-se_.`,
					admin: '*ESTE COMANDO SÓ PODE SER USADO POR ADMINISTRADORES DO GRUPO, NEM SEI O MOTIVO DE VOCÊ TER TENTADO USÁ-LO MESMO SABENDO QUE NÃO IREI OBEDECER 😡*',
					Badmin: '*Este comando só pode ser usado quando eu me torno admin do grupo!*',
					publikG: `O bot agora está privado\nPara saber mais digite  \n*${prefix}infobot*`
				}
			}

			const botNumber = denz.user.jid
			const ownerNumber = ["557491143362@s.whatsapp.net","557491143362@s.whatsapp.net","557491143362@s.whatsapp.net"] // owner number ubah aja
			...
			const checkLimit = (sender) => {
				...
				try {
					const getmemex = groupMembers.length
					    ...
					    denz.sendMessage(from, `Desculpe, não posso permanecer no grupo por conta da quantidade de membros *${groupMetadata.subject}*Não atende a quantidade minima de membros.\n\n Quantidade mínima:*${memberLimit}*`, text)
					setTimeout( () => {
		...
		setTimeout( () => {
		reply(`*Android No.18*\n\nSairei do grupo em breve*${groupMetadata.subject}*`)
		}, 0)
		}
...
denz.sendMessage(from, hasil, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "ketik #menu kak", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
		...
		if (isGroupAdmins) return reply(`*${pushname2}*Isso é um link. Ah é, você é um admin e só por isso não irei te remover.*`)
		denz.updatePresence(from, Presence.composing)
		...
		setTimeout( () => {
		reply(`*Achei isso ofensivo*\nAdeus*${pushname2}* Da próxima vez aprenda a respeitar as outras pessoas.*${groupMetadata.subject}*`)
		}, 0)
		}
			...
			if (!isUser) return reply(mess.only.userB)
				dmenu = `*Menu Android Número 18 🌙*

║▪︎ *Prefixo: ${prefix}*
║▪︎ *Usuário registrado:* ${user.length}
...
║▪︎ ${prefix}ɢʜᴏꜱᴛ
║▪︎ ${prefix}ᴛᴏᴘᴛᴛ
║▪︎ ${prefix}ғᴀᴋᴇᴀᴅᴅʀᴇꜱꜱ
║▪︎ ${prefix}ɢᴏᴏɢʟᴇꜱᴇᴀʀᴄʜ
║▪︎ ${prefix}ʜʙᴅ 
║▪︎ ${prefix}ꜱʜɪɴᴇ 
║▪︎ ${prefix}ᴛʜᴇɴɪɢʜᴛ 
║▪︎ ${prefix}ʀᴀɪɴʙᴏᴡ 
...
║▪︎ ${prefix}ꜱʜᴏʀᴛʟɪɴᴋ2
║▪︎ ${prefix}ꜱʜᴏʀᴛʟɪɴᴋ3
║▪︎ ${prefix}ᴀꜱᴍᴀᴜʟʜᴜꜱɴᴀ
║▪︎ ${prefix}ᴄᴏᴠɪᴅɪɴᴅᴏ
║▪︎ ${prefix}ᴄᴏᴠɪᴅɢʟᴏʙᴀʟ
║▪︎ ${prefix}ɪɴғᴏɢᴇᴍᴘᴀ
║▪︎ ${prefix}ʀᴀɴᴅᴏᴍᴡᴘ
║▪︎ ${prefix}ɢᴀʙᴜᴛ
║▪︎ ${prefix}ϙᴜᴏᴛᴇᴍᴜꜱʟɪᴍ
║▪︎ ${prefix}ʙᴀᴛᴛʟᴇғɪᴇʟᴅ
║▪︎ ${prefix}ᴘᴜʙɢʟᴏɢᴏ
║▪︎ ${prefix}ʙᴜᴛᴛᴇʀғʟʏ
║▪︎ ${prefix}ᴋᴏᴅᴇᴘᴏꜱ
║▪︎ ${prefix}ʟᴀᴄᴀᴋɪᴘ
...
║▪︎ ${prefix}ᴡᴀɪᴛ
║▪︎ ${prefix}ᴊᴀᴅᴡᴀʟᴛᴠ 
║▪︎ ${prefix}ᴊᴀᴅᴡᴀʟᴛᴠɴᴏᴡ
║▪︎ ${prefix}ᴊᴀᴅᴡᴀʟʙᴏʟᴀ
║▪︎ ${prefix}ʟɪʀɪᴋ 
║▪︎ ${prefix}ᴄʜᴏʀᴅ 
║▪︎ ${prefix}ᴡɪᴋɪ 
║▪︎ ${prefix}ʙʀᴀɪɴʟʏ 
║▪︎ ${prefix}ʀᴇꜱᴇᴘᴍᴀꜱᴀᴋᴀɴ 
║▪︎ ${prefix}ʙᴇʀɪᴛᴀʜᴏᴀx
║▪︎ ${prefix}ᴍᴀᴘ 
║▪︎ ${prefix}ғɪʟᴍ 
║▪︎ ${prefix}ᴊᴀʀᴀᴋ
...
║▪︎ ${prefix}ɢᴀɴᴛᴇɴɢ
║▪︎ ${prefix}ᴀɴᴛɪғᴀᴋᴇ 1/0
║▪︎ ${prefix}ʙᴇʙᴀɴ
║▪︎ ${prefix}ᴏᴘᴇɴᴛɪᴍᴇ
║▪︎ ${prefix}ᴄʟᴏꜱᴇᴛɪᴍᴇ
║▪︎ ${prefix}ᴍᴏᴅᴇᴀɴɪᴍᴇ 
║▪︎ ${prefix}ᴀɴᴛɪʟɪɴᴋ
║▪︎ ${prefix}ᴀɴᴛɪᴘᴀʟᴀᴠʀᴀᴏ
║▪︎ ${prefix}ʟɪsᴛᴘᴀʟᴀᴠʀᴀ
║▪︎ ${prefix}ʙᴇᴍᴠɪɴᴅᴏ
║▪︎ ${prefix}ɢʀᴜᴘ
║▪︎ ${prefix}ᴏᴡɴᴇʀɢʀᴜᴘ
║▪︎ ${prefix}ꜱᴇᴛᴘᴘ
...
║▪︎ ${prefix}ᴄᴇʀꜱᴇx
║▪︎ ${prefix}ʀᴀɴᴅᴏᴍʙᴏᴋᴇᴘ
║▪︎ ${prefix}ᴘᴏʀɴʜᴜʙ
║▪︎ ${prefix}xᴠɪᴅᴇᴏꜱ
║▪︎ ${prefix}ɴꜱғᴡʟᴏʟɪ
║▪︎ ${prefix}ɴꜱғᴡʙʟᴏᴡᴊᴏʙ
║▪︎ ${prefix}ɴꜱғᴡɴᴇᴋᴏ
║▪︎ ${prefix}ɴꜱғᴡᴛʀᴀᴘ
║▪︎ ${prefix}ʜᴇɴᴛᴀɪ
║▪︎ ${prefix}ɴᴇᴋᴏᴘᴏɪ
...
denz.sendMessage(from, dmenu, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "Modificações feitas por 🌙 Haru", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('dnsnew.jpg')}}}})
				...
				case 's':
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`\`\`\`Falha, no momento da conversão de ${tipe} para sticker\`\`\``)
							})
							.on('end', function () {
								console.log('Finish')
								buff = fs.readFileSync(ran)
...
case 'igpost': 				
		...
		case 'covidglobal': 
				...
				break
		case 'hadits': 
				if (args.length < 1) return reply(`kitab + nomor nya mana om? Exemplo: hadits muslim|116`)
				ct = body.slice(8)
				ll1 = ct.split("|")[0];
                ll2 = ct.split("|")[1];
				reply(`[❕] Loading`)
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}gsuggest dns/bot/wea`)
du = `${body.slice(10)}`
ted1 = du.split("/")[0];
ted2 = du.split("/")[1];
ted3 = du.split("/")[2];
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}candy dns`)
cndy = body.slice(7)
reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}hpotter dns`)
hpter = body.slice(9)
reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}woodblock dns`)
woblk = body.slice(11)
reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}galaxtext dns`)
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}pupycute dns`)
puki = body.slice(10)
reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}galaxstyle dns`)
gelay = body.slice(12)
reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}hologram dns`)
gram = body.slice(10)
reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}metallogo dns`)
metal = body.slice(11)
reply(mess.wait)
...
if (isBanned) return reply(mess.only.benned)    
if (!isUser) return reply(mess.only.userB)
if (args.length < 1) return reply(`Exemplo: ${prefix}bpink dns`)
black = body.slice(7)
reply(mess.wait)
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
            ...
            break  
                 ...
                 case 'kalkulator':
				   ...
				   if (isLimit(sender)) return reply(limitend(pushname2))
				     if (args.length < 1) return reply(`[❗] Enviar comando *${prefix} calculator [Number]* \nExemplo: ${prefix} calculator 12*12 \n* NOTA *:\n- Para multiplicação usando *\n- Para aumentar usando +\n- Para redução usando -\n- Para compartilhar usando /`)
				    mtk = `${body.slice(12)}`
         ...
         case 'lirik':
                ...
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
                if (args.length < 1) return reply(`Yang mau direport apaan? Exemplo: #report fitur #hentai error`)
          				
                     const pesan = body.slice(8)
                      if (pesan.length > 300) return denz.sendMessage(from, 'Desculpe, o texto é muito longo, máximo de 300 textos', text, {quoted: mek})
                    ...
                    denz.sendMessage('557491143362@s.whatsapp.net', options, text, {quoted: mek})
                    reply('Problemas foram relatados ao proprietário do BOT, relatórios falsos não serão respondidos.')
                    break
                case 'reqjdjdjdjxuest':
                if (isBanned) return reply(mess.only.benned)    
                if (!isUser) return reply(mess.only.userB)
                if (args.length < 1) return reply(`O que você deseja solicitar? Exemplo: !request recursos de anime`)
          				
                     const cfrr = body.slice(8)
                      if (cfrr.length > 300) return denz.sendMessage(from, 'Desculpe, o texto é muito longo, máximo de 300 textos', text, {quoted: mek})
                        var nomor = mek.participant
                       const ress = `*[Pedir recurso]*\nNúmero : @${nomor.split("@s.whatsapp.net")[0]}\nPesan : ${cfrr}`
...
//creator
			...
			reply(mess.wait)
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					wm = gh.split("/")[1];
					bg = gh.split("/")[2];
					const pref = `Usage: \n${prefix}quotemaker teks/watermark/theme\n\nEx :\n${prefix}quotemaker ini Exemplo/bicit/random`
...
//akhir kreator
			...
			case 'katailham':
				...
				svst = body.slice(12)
				if (!svst) return reply('Qual é o nome da figurinha?')
				...
				case 'addaudio':
                   if (!isOwner) return reply(mess.only.ownerB)
                   if (!isQuotedAudio) return reply('Responda o áudio!')
                   gsh = body.slice(10)
                   if (!gsh) return reply('Qual o nome do áudio?')
			...
			case 'bucinpersen':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				if (args.length < 1) return reply('Digite o nome!')
				rate = body.slice(8)
...
quoted: mek, caption: `Hasil Dari *${daerah}*`
  ...
  break
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`o/*${pushname2}* `+ teks +' *ANDROID NO.18 🌙', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 *ANDROID NO.18 🌙* 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`*Todas as conversas foram apagadas com sucesso* ✅ \`\`\``)
					break
                                case 'bcgc':
					denz.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of groupMembers) {
							denz.sendMessage(_.jid, bcgc, image, {caption: `*Mensagem de transmissão*\n*ɢʀᴜᴘᴏ* : ${groupName}\n\n${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of groupMembers) {
							sendMess(_.jid, `⚠️ *_Transmissão de alerta_* ⚠️\n*ɢʀᴜᴘᴏ* : ${groupName}\n\n${body.slice(6)}`)
						}
				...
				if (!isUser) return reply(mess.only.userB)
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar, talvez porque o número está privado')
					}
					break
			    case 'remover':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Removido com sucesso ✅@${_.split('@')[0]}\n`
						}
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque a pessoa que você deseja promover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						...
						mentions(`Parabés! @${mentioned[0].split('@')[0]} Agora você é um administrador do grupo *${groupMetadata.subject}*`, mentioned, true)
						denz.groupMakeAdmin(from, mentioned)
					}
					break
				case 'delete':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque a pessoa que você deseja remover da lista de admins!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Removido da lista de admins com sucesso:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido recebido, despromovido: @${mentioned[0].split('@')[0]} agora é um membro comum!`, mentioned, true)
						denz.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'listadmins':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista admins deste grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`Modo Simi ativado com sucesso\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						samih.splice(from, 1)
						fs.writeFileSync('./database/json/simi.json', JSON.stringify(samih))
						reply(`\`\`\`âœ“Modo Simi desativado com sucesso!\`\`\` *${groupMetadata.subject}*`)
					} else {
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if ((args[0]) === 'on') {
						...
						reply(`\`\`\`✓“Modo anime ativado com sucesso!\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						anime.splice(from, 1)
						fs.writeFileSync('./database/json/anime.json', JSON.stringify(anime))
						reply(`\`\`\`✓“Modo anime desativado com sucesso!\`\`\` *${groupMetadata.subject}*`)
					} else {
				...
				if (!isGroup) return reply(mess.only.group)
					...
					if ((args[0]) === 'on') {
						if (isAntiLink) return reply('O anti-link já está ativo!')
						antilink.push(from)
						fs.writeFileSync('./database/json/antilink.json', JSON.stringify(antilink))
						reply(`\`\`\`✓“Ativou com sucesso o recurso anti-link no grupo\`\`\` *${groupMetadata.subject}*`)
					} else if ((args[0]) === 'off') {
						if (!isAntiLink) return reply('O anti-link já está desativado')
				...
				case 'antipalavrao':
                    if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
                if (args.length < 1) return reply('on para ligar, off para desabilitar')
                if (args[0] === 'on') {
                if (isBadWord) return reply('anti-palavrão já on')
                 	   badword.push(from)
                 	   fs.writeFileSync('./database/json/badword.json', JSON.stringify(badword))
                  	   reply(`\`\`\`✓“Anti-ofensa ativado com sucesso!\`\`\` *${groupMetadata.subject}*`)
              	  } else if (args[0] === 'off') {
                    ...
                    case 'addpalavra':
                    if (!isOwner) return reply(mess.only.ownerB)
                    if (args.length < 1) return reply( `Enviar pedidos ${prefix}addpalavra [insultos ou palavrões]. exemplo ${prefix}addpalavra (palavra que deseja adiciona na lista)`)
                ...
                case 'delpalavra':
                    if (!isOwner) return reply(mess.only.ownerB)
                    if (args.length < 1) return reply( `Remover palavras ${prefix}delpalavra (insulto ou xingamento) .Exemplo ${prefix}delpalavra (palavra que deseja remover da lista)`)
                    ...
                    break 
                case 'listpalavra':
                    let lbw = `Lista de palavras proíbidas por mim\nTotal : ${bad.length}\n`
                    for (let i of bad) {
                        lbw += `➸ ${i.replace(bad)}\n`
                    }
                    await reply(lbw)
...
//setting bot
				...
				case 'setnamebot':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					name = body.slice(12)
					reply(`Nome alterado com sucesso para : ${name}`)
					break 
				case 'setreply':
					if (!isOwner) return reply(mess.only.ownerB)
                    denz.updatePresence(from, Presence.composing) 
...
////////////
	...
	case 'infocuaca':
     ...
     if (isLimit(sender)) return reply(limitend(pushname2))
     if (args.length < 1) return reply(from, 'Envie o comando *!cuaca [tempat]*\nExemplo : *!cuaca Canada', text)
     reply(mess.wait)
...
case 'husbu':
		...
		if (budy.includes(`Thanks`)) {
                  ...
                  if (budy.includes(`thanks`)) {
                  reply(`Por nada! *${pushname2}*, sᴄʀᴇᴀᴍᴏ sᴀᴜᴅᴀᴅᴇs >-<`)
                  }
                  if (budy.includes(`makasih`)) {
                  reply(`Por nada! *${pushname2}*, sᴄʀᴇᴀᴍᴏ sᴀᴜᴅᴀᴅᴇs >-<`)
                  }
...
denz.sendMessage(from, comd, text, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "tidak ada dalam #menu", 'jpegThumbnail': fs.readFileSync('./sticker/dnsnew.webp')}}}})
                  ...
                  if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[db]','aqua'), '*Comando não registrado, digite !menu no chat privado para receber a lista com os comandos, caso você envie no grupo você será removido imediatamente e bloqueado!*', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
